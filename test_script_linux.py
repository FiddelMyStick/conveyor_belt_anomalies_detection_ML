...
try:
    from resnet import ResNetVAE_V2
except ImportError:
...
# --- Threaded Video Stream Reader (Unchanged) --- #
...
# --- Main Analysis Function (MODIFIED WITH LIVE FEEDBACK) --- #
def analyse_stream(args):
    ...
    print(f"Anomaly Threshold ({score_name}): {args.threshold}")
    ...
    print("\nStarting continuous analysis... Press Ctrl+C to stop.")
    try:
        while True:
            ...
            if source_frame_count % args.frame_sampling_rate != 0: continue
            
            analyzed_frame_count += 1 # Increment counter for analyzed frames
            ...
            if score_to_use >= args.threshold:
                ...
                # +++ NEW: ANOMALY ALERT PRINTOUT +++
                print(f"-> ANOMALY DETECTED! Frame Source: {source_frame_count}, {score_name}: {score_to_use:.4f} (Threshold: {args.threshold})")
            ...
            # +++ NEW: PERIODIC STATUS UPDATE +++
            if analyzed_frame_count % args.status_interval == 0 and analyzed_frame_count > 0:
                # Calculate rolling average FPS on the last N processing times for more current reading
                rolling_avg_time = np.mean(processing_times[-args.status_interval:])
